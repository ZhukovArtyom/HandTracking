import React, { useState, useEffect } from 'react';
import ActionCard from './ActionCard';
import action_img from '../assets/action_icon.png';
import keys_img from '../assets/keys.png';
import exe_img from '../assets/exe_icon.png';
import system_com_img from '../assets/system_com_icon.png';

import keyboardCommandsData from '../../../backend/config/keyboard_commands.json';
import installedProgramsData from '../../../backend/config/installed_programs.json';
import systemCommandsData from '../../../backend/config/system_commands.json';

function ActionLib({ onSelectAction }) {
  const [keyboardCommands, setKeyboardCommands] = useState([]);
  const [installedPrograms, setInstalledPrograms] = useState([]);
  const [systemCommands, setSystemCommands] = useState([]);

  const [searchTerm, setSearchTerm] = useState('');
  const [loading, setLoading] = useState(true);

  // Загрузка данных
  useEffect(() => {
    loadData();
  }, []);

  const loadData = async () => {
    setLoading(true);
    try {
      // Загружаем клавиши
      setKeyboardCommands(keyboardCommandsData);
      // Загружаем программы
      setInstalledPrograms(installedProgramsData);

      setSystemCommands(systemCommandsData);

    } catch (error) {
      console.error('Ошибка загрузки данных:', error);
      setKeyboardCommands([]);
      setInstalledPrograms([]);
      setSystemCommands([]);
    } finally {
      setLoading(false);
    }
  };

   const handleCardClick = (displayName, on_press, on_release, type) => {
     if (onSelectAction) {
       onSelectAction({
         displayName: displayName,
         on_press: on_press,
         on_release: on_release,
         type: type
       });
     }
   };

  // Фильтрация команд по поиску
  const filteredCommands = keyboardCommands.filter(command =>
    command.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Фильтрация программ по поиску
  const filteredPrograms = installedPrograms.filter(program =>
    program.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Фильтрация команд по поиску
  const filteredSystemCommands = systemCommands.filter(command =>
    command.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="h-[47vmax] w-full bg-white border-t-2 border-gray-300 -mt-[7vmax] flex flex-col shadow-[0px_-2px_8px_rgba(0,0,0,0.25)]">
      <div className="h-[6vmax] w-full flex items-center justify-between py-[1.2vmax] px-[2vmax] border-b-2 border-gray-300">
        <div className="h-full flex items-center">
          <img src={action_img} className="h-full mr-[1vmax]" alt=""/>
          <p className="text-[2vmax] font-semibold">Библиотека действий</p>
        </div>
        <input
          type="text"
          placeholder="Поиск..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="h-full text-[1.7vmax] p-[1.5vmax] border border-[rgb(6,207,249)] rounded-lg focus:outline-none focus:ring-1 focus:ring-[rgb(6,207,249)]"
        />
      </div>

      <div className="flex-1 min-h-0 flex pr-[2vmax]">
        {/* Блок клавиш и сочетаний клавиш */}
        <div className="h-full w-1/3 flex flex-col">
          <div className="h-[3vmax] w-full flex items-center px-[2vmax] my-[2vmax]">
            <img src={keys_img} className="h-full" alt=""/>
            <p className="text-[1.6vmax] font-semibold pl-[2vmax]">
              Клавиши <br /> / Сочетания клавиш
            </p>
          </div>

          <div className="h-full flex flex-col overflow-y-auto pt-[0.5vmax] pl-[2vmax] pr-[1vmax]">
            {loading ? (
              <div className="text-center text-gray-500">Загрузка...</div>
            ) : filteredCommands.length > 0 ? (
              filteredCommands.map((command, index) => (
                <ActionCard
                  key={index}
                  command={command}
                  onClick={() => handleCardClick(command, command, command, 'keyboard')}
                />
              ))
            ) : (
              <div className="text-center text-gray-500">
                {searchTerm ? 'Ничего не найдено' : 'Нет доступных команд'}
              </div>
            )}
          </div>
        </div>

        {/* Блок открытия программ */}
        <div className="h-full w-1/3 flex flex-col">
          <div className="h-[3vmax] w-full flex items-center px-[2vmax] my-[2vmax]">
            <img src={exe_img} className="h-full" alt=""/>
            <p className="text-[1.7vmax] font-semibold pl-[2vmax]">Открытие программ</p>
          </div>
          <div className="h-full flex flex-col overflow-y-auto pt-[0.5vmax] pl-[2vmax] pr-[1vmax]">
            {loading ? (
              <div className="text-center text-gray-500">Загрузка...</div>
            ) : filteredPrograms.length > 0 ? (
              filteredPrograms.map((program, index) => (
                <ActionCard
                  key={index}
                  command={program.name}
                  onClick={() => handleCardClick(program.name, program.path,'', 'program')}

                />
              ))
            ) : (
              <div className="text-center text-gray-500">
                {searchTerm ? 'Ничего не найдено' : '...'}
              </div>
            )}
          </div>
        </div>

        {/* Блок системных команд */}
        <div className="h-full w-1/3 flex flex-col">
          <div className="h-[3vmax] w-full flex items-center px-[2vmax] my-[2vmax]">
            <img src={system_com_img} className="h-full" alt=""/>
            <p className="text-[1.6vmax] font-semibold pl-[2vmax]">
              Мышь <br /> / Системные команды
            </p>
          </div>
          <div className="h-full flex flex-col overflow-y-auto pt-[0.5vmax] pl-[2vmax] pr-[1vmax]">
                {loading ? (
                  <div className="text-center text-gray-500">Загрузка...</div>
                ) : filteredSystemCommands.length > 0 ? (
                  filteredSystemCommands.map((command, index) => (
                    <ActionCard
                      key={index}
                      command={command.name}
                      onClick={() => handleCardClick(command.name, command.on_press, command.on_release, 'system')}
                    />
                  ))
                ) : (
                  <div className="text-center text-gray-500">
                    {searchTerm ? 'Ничего не найдено' : 'Нет доступных команд'}
                  </div>
                )}
          </div>
        </div>
      </div>
    </div>
  );
}

export default ActionLib;